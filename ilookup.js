var is_ilookup_script_already_loaded = is_ilookup_script_already_loaded || false;


(function ($) {


  if( is_ilookup_script_already_loaded )
  {
    return;
  }
  else
  {
    is_ilookup_script_already_loaded = true;
  }
  ...
  $.fn.iLookup = function(options) {
    ...
    this.each(function(){
      ...
      if($t.attr('data-ilookup_initialized') == '1')
      {
        return true;
      }

      $t.attr('data-ilookup_initialized', '1');


      var input_group = $(`<div class="input-group">
                          </div>`);

      var hidden_input = $(`<input type="hidden" name="`+$t.data('name')+`" value="`+$t.data('id')+`">`);
        ...
        if(!options.one_field_search)
        {
          if(col.is_searchable == true)
          {
            search_form.append( $(`<div class="form-group">
                                    ...
                                    <input id="`+col.col_name+`" type="text" class="form-control input-sm" name="`+col.col_name+`">
                                  </div>`) );
      ...
      var search_btn = $(`<div class="form-group">
                            <label>&nbsp;</label>
      ...
      var showModalAndLoadAjax = function(){
      ...
      result_table.on('click', '.btn-select, tbody tr', function(e){
        e.stopPropagation();
      ...
      function get_ajax_response(ajax_url)
      {
        ...
        request.done(function(data){
          $.each(data.data, function(i, data){
              ...
              //select button
              var select_button = $('<td><button class="btn btn-primary btn-xs btn-select">Select</button></td>');
              select_button.find('.btn-select').data('obj', data);
              row.append(select_button);
              select_button.closest('tr').data('obj', data);
